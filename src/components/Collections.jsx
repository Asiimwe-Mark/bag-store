...
const Collections = ({ products = [], onCollectionClick }) => {
  // Show up to 5 most recently added products
        ...
        <div className="mb-10 sm:mb-16 fade-up stagger-2">
          ...
          <div className="collection-scroll flex gap-4 sm:gap-6 overflow-x-auto pb-4">
            {newArrivals.map((product, index) => (
              <div
                key={product.id}
                onClick={() => onCollectionClick({ name: product.name, price: formatPrice(product.price), image: product.images?.[0] })}
              ...
              >
                <div className="relative h-60 sm:h-72 overflow-hidden img-zoom-container">
                  ...
                  <div className="collection-overlay absolute inset-0 flex flex-col justify-end p-4 sm:p-6">
                    {product.brand && (
                      <span className="text-xs font-bold text-champagne-300 tracking-widest uppercase mb-1">{product.brand}</span>
                    )}
                    <h4 className="font-serif text-lg sm:text-xl font-bold text-white">{product.name}</h4>
                    <p className="text-white/80 text-xs sm:text-sm mt-1">{formatPrice(product.price)}</p>
